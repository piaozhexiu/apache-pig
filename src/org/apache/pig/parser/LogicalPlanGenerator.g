...
// Set to false if schema is parsed from @OutputSchema and all fields need to be unique,
// so aliases for unnamed fields will be generated by OutputSchemaResolver and not
// by NumValCarrier.
private boolean generateFieldName = true;
public void setGenerateFieldName(boolean generate) {
  generateFieldName = generate;
}
...
@init {
 ...
 | ^( FIELD_DEF_WITHOUT_IDENTIFIER ( type { datatype = $type.datatype; } ) )
   {
           $fieldSchema = new LogicalFieldSchema ( ($nvc == null ? null : $nvc.makeNameFromDataType(datatype)) , $type.logicalSchema, datatype );
   }
;
...
@init {
    ...
    NumValCarrier nvc = generateFieldName ? new NumValCarrier() : null;
}
