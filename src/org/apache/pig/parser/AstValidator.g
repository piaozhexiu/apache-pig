...
// If schema is parsed from @OutputSchema, duplicate check is taken place there, so here
// it is disabled
private boolean validateSchemaAlias = true;
public void setValidateSchemaAlias(boolean validate) {
  validateSchemaAlias = validate;
}
...
field_def[Set<String> fieldNames, NumValCarrier nvc] throws DuplicatedSchemaAliasException
 : ^( FIELD_DEF IDENTIFIER { if(validateSchemaAlias) validateSchemaAliasName( fieldNames, $IDENTIFIER, $IDENTIFIER.text ); } type? )
 | ^( FIELD_DEF_WITHOUT_IDENTIFIER type { if(validateSchemaAlias) validateSchemaAliasName ( fieldNames, $FIELD_DEF_WITHOUT_IDENTIFIER, ( generateFieldName ? $nvc.makeNameFromDataType ( $type.typev ) : null ) ); } )
;
